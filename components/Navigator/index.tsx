      ...
      <ul className="flex items-center">
        ...
        <li>
          {lang.iso === 'PT' ? (
            <GiBrazilFlag
              onClick={toggleLang}
              className="bg-gradient-to-r from-IRON text-white to-IRON dark:from-white dark:text-IRON dark:to-white hover:from-EMERALD hover:text-amber-300 hover:to-sky-600 hover:dark:from-EMERALD hover:dark:text-amber-300 hover:dark:to-sky-600  px-1 max-h-8 rounded-md cursor-pointer text-4xl transition-colors ease-in-out ml-4 md:ml-8"
            />
          ) : (
            <LiaFlagUsaSolid
              onClick={toggleLang}
              className="text-ROSE dark:text-CYAN cursor-pointer text-4xl hover:text-ORANGE dark:hover:text-EMERALD transition-colors ease-in-out ml-4 md:ml-8"
            />
          )}
        </li>
      </ul>
